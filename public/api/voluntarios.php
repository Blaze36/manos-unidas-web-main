...
function createVoluntario($db) {
    ...
    $missing = validateRequired($data, ['nombre_completo', 'email', 'password', 'telefono', 'ciudad', 'disponibilidad']);
    if (!empty($missing)) {
        jsonResponse([
            'success' => false,
    ...
    try {
        ...
        $queryPersona = "INSERT INTO personas 
                        (nombre_completo, email, password, telefono, ciudad, direccion, acepta_privacidad, recibe_newsletter) 
                        VALUES 
                        (:nombre, :email, :password, :telefono, :ciudad, :direccion, :privacidad, :newsletter)";
        ...
        $stmtPersona->bindParam(':password', $passwordHash);
        ...
        if ($newsletter) {
            $queryNews = "INSERT INTO suscriptores_newsletter (email, id_persona) 
                         ...
                         ON DUPLICATE KEY UPDATE id_persona = :id_persona_update";
            $stmtNews = $db->prepare($queryNews);
            $stmtNews->bindParam(':email', $email);
            $stmtNews->bindParam(':id_persona', $personaId);
            $stmtNews->bindParam(':id_persona_update', $personaId);
            $stmtNews->execute();
        }
