...
async function handleRegistroSubmit(event) {
    ...
    if (!nombre || !email || !telefono || !ciudad || !ayuda || !disponibilidad) {
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (target) {
                ...
                // Si es el dashboard, cargar automáticamente
    ...
    // Si ya estamos en el dashboard (por hash en URL), cargarlo
    ...
    if (formLogin) {
        formLogin.addEventListener('submit', async function(event) {
            ...
            try {
                ...
                console.error('Error:', error);
                showToast(error.message || 'No se encontró una cuenta con ese correo', 'error');
            } finally {
                submitBtn.classList.remove('btn-loading');
                submitBtn.disabled = false;
            }
