...
<head>
    ...
    <style>
        ...
        .dash-table tr:hover td { background:#e0f2fe; }
        .dash-table td, .dash-table th { padding:8px 12px; font-size:.84rem; }
        .bar-wrap   { display:flex; align-items:center; gap:8px; margin-bottom:9px; }
        .bar-label  { width:120px; font-size:.78rem; color:#475569; text-align:right; flex-shrink:0; }
        .bar-track  { flex:1; background:#e2e8f0; border-radius:4px; height:20px; overflow:hidden; }
        .bar-fill   { height:100%; border-radius:4px; transition:width .8s ease; display:flex;
                      align-items:center; justify-content:flex-end; padding-right:6px;
                      font-size:.7rem; color:#fff; font-weight:700; white-space:nowrap; }
        .rep-th { background:#0f766e; color:#fff; font-size:.8rem; padding:9px 12px; text-align:left; whitespace:nowrap; }
        .rep-td { padding:8px 12px; font-size:.82rem; border-bottom:1px solid #e2e8f0; color:#334155; }
        tr:hover .rep-td { background:#f0fdfa; }
        .filter-label { font-size:.8rem; color:#475569; display:block; margin-bottom:3px; font-weight:600; }
        .filter-ctrl  { border:1px solid #cbd5e1; border-radius:6px; padding:6px 10px;
                        font-size:.82rem; color:#1e293b; background:#fff; }
        .rep-tab { padding:8px 18px; font-size:.85rem; font-weight:600; border-bottom:2px solid transparent;
                   color:#6b7280; cursor:pointer; transition:all .2s; }
        .rep-tab:hover  { color:#0f766e; }
        .rep-tab.active { border-color:#0f766e; color:#0f766e; }
...
<nav class="bg-teal-800 text-white sticky top-0 z-10 shadow-lg">
    <div class="container mx-auto px-4">
        <div class="flex flex-wrap justify-center">
            ...
            <a href="#dashboard" class="nav-link px-4 py-3 hover:bg-teal-600 transition duration-300 flex items-center gap-1"><i class="fas fa-chart-line text-xs"></i> Dashboard</a>
...
<main class="container mx-auto px-4 py-8">
    ...
    <!-- DASHBOARD -->
    <section id="dashboard" class="mb-16 bg-white rounded-lg shadow-md p-6">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Resumen en tiempo real de la actividad de la ONG</p>
            </div>
            <div class="flex items-center gap-3">
                <span id="dash-updated" class="text-xs text-gray-400 italic"></span>
                <button onclick="loadDashboard()" class="bg-teal-700 hover:bg-teal-800 text-white text-sm px-4 py-2 rounded-lg flex items-center gap-2 transition duration-300">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="stat-card bg-teal-50 border border-teal-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-teal-700 font-bold uppercase tracking-wide">Personas</span>
                    <div class="bg-teal-100 p-2 rounded-full"><i class="fas fa-users text-teal-600"></i></div>
                </div>
                <div id="kpi-personas" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">registradas en total</p>
            </div>
            <div class="stat-card bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-blue-700 font-bold uppercase tracking-wide">Voluntarios</span>
                    <div class="bg-blue-100 p-2 rounded-full"><i class="fas fa-hand-holding-heart text-blue-600"></i></div>
                </div>
                <div id="kpi-voluntarios" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">activos</p>
            </div>
            <div class="stat-card bg-orange-50 border border-orange-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-orange-700 font-bold uppercase tracking-wide">Donaciones</span>
                    <div class="bg-orange-100 p-2 rounded-full"><i class="fas fa-donate text-orange-600"></i></div>
                </div>
                <div id="kpi-donaciones" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">registradas</p>
            </div>
            <div class="stat-card bg-green-50 border border-green-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-green-700 font-bold uppercase tracking-wide">Lempiras</span>
                    <div class="bg-green-100 p-2 rounded-full"><i class="fas fa-coins text-green-600"></i></div>
                </div>
                <div id="kpi-lempiras" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">donados en total</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div class="stat-card bg-purple-50 border border-purple-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-purple-700 font-bold uppercase tracking-wide">Beneficiarios</span>
                    <div class="bg-purple-100 p-2 rounded-full"><i class="fas fa-heart text-purple-600"></i></div>
                </div>
                <div id="kpi-beneficiarios" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">activos</p>
            </div>
            <div class="stat-card bg-pink-50 border border-pink-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-pink-700 font-bold uppercase tracking-wide">Actividades</span>
                    <div class="bg-pink-100 p-2 rounded-full"><i class="fas fa-calendar-check text-pink-600"></i></div>
                </div>
                <div id="kpi-actividades" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">activas</p>
            </div>
            <div class="stat-card bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-yellow-700 font-bold uppercase tracking-wide">Pendientes</span>
                    <div class="bg-yellow-100 p-2 rounded-full"><i class="fas fa-clock text-yellow-600"></i></div>
                </div>
                <div id="kpi-pendientes" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">donaciones por atender</p>
            </div>
            <div class="stat-card bg-cyan-50 border border-cyan-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs text-cyan-700 font-bold uppercase tracking-wide">Newsletter</span>
                    <div class="bg-cyan-100 p-2 rounded-full"><i class="fas fa-envelope text-cyan-600"></i></div>
                </div>
                <div id="kpi-suscriptores" class="text-3xl font-bold text-gray-800">—</div>
                <p class="text-xs text-gray-500 mt-1">suscriptores activos</p>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h3 class="font-semibold text-gray-700 mb-4 flex items-center gap-2">
                    <i class="fas fa-list text-orange-500"></i> Últimas donaciones
                </h3>
                <div class="overflow-x-auto">
                    <table class="w-full dash-table">
                        <thead><tr class="text-left text-xs text-gray-500 border-b border-gray-200">
                            <th class="pb-2 px-2">Persona</th>
                            <th class="pb-2 px-2">Tipo</th>
                            <th class="pb-2 px-2">Cant.</th>
                            <th class="pb-2 px-2">Estado</th>
                            <th class="pb-2 px-2">Fecha</th>
                        </tr></thead>
                        <tbody id="dash-tabla-donaciones">
                            <tr><td colspan="5" class="text-center text-gray-400 py-6 text-sm">Haz clic en Actualizar para cargar datos</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h3 class="font-semibold text-gray-700 mb-4 flex items-center gap-2">
                    <i class="fas fa-user-plus text-blue-500"></i> Últimas personas registradas
                </h3>
                <div class="overflow-x-auto">
                    <table class="w-full dash-table">
                        <thead><tr class="text-left text-xs text-gray-500 border-b border-gray-200">
                            <th class="pb-2 px-2">Nombre</th>
                            <th class="pb-2 px-2">Ciudad</th>
                            <th class="pb-2 px-2">Tipo</th>
                            <th class="pb-2 px-2">Fecha</th>
                        </tr></thead>
                        <tbody id="dash-tabla-personas">
                            <tr><td colspan="4" class="text-center text-gray-400 py-6 text-sm">Haz clic en Actualizar para cargar datos</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        <div class="grid md:grid-cols-2 gap-6 mb-6">
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h3 class="font-semibold text-gray-700 mb-4 flex items-center gap-2">
                    <i class="fas fa-chart-bar text-teal-600"></i> Donaciones por tipo de artículo
                </h3>
                <div id="dash-chart-tipos"><p class="text-gray-400 text-sm text-center py-4">Sin datos aún</p></div>
            </div>
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h3 class="font-semibold text-gray-700 mb-4 flex items-center gap-2">
                    <i class="fas fa-clock text-blue-500"></i> Voluntarios por disponibilidad
                </h3>
                <div id="dash-chart-disponibilidad"><p class="text-gray-400 text-sm text-center py-4">Sin datos aún</p></div>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h3 class="font-semibold text-gray-700 mb-4 flex items-center gap-2">
                    <i class="fas fa-tasks text-yellow-500"></i> Donaciones por estado
                </h3>
                <div id="dash-chart-estados"><p class="text-gray-400 text-sm text-center py-4">Sin datos aún</p></div>
            </div>
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h3 class="font-semibold text-gray-700 mb-4 flex items-center gap-2">
                    <i class="fas fa-envelope-open-text text-cyan-600"></i> Últimos suscriptores newsletter
                </h3>
                <div class="overflow-x-auto">
                    <table class="w-full dash-table">
                        <thead><tr class="text-left text-xs text-gray-500 border-b border-gray-200">
                            <th class="pb-2 px-2">Email</th>
                            <th class="pb-2 px-2">Vinculado</th>
                            <th class="pb-2 px-2">Fecha</th>
                        </tr></thead>
                        <tbody id="dash-tabla-newsletter">
                            <tr><td colspan="3" class="text-center text-gray-400 py-6 text-sm">Sin datos aún</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        <div class="grid md:grid-cols-4 gap-8 mb-8">
            ...
            <div>
                ...
                <ul class="space-y-2 text-teal-300">
                    ...
                    <li>Sábados: 9:00 - 12:00</li>
                    <li>Domingos: Cerrado</li>
                </ul>
            </div>
        </div>
        <div class="border-t border-teal-700 pt-6 text-center text-teal-300">
...
<div id="modal-login" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        ...
        <p class="text-gray-600 mb-6">Ingresa tu correo electrónico para continuar</p>
